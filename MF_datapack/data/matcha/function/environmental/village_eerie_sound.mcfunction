...
execute at @a run execute if score @p eerie >= 1 eerie run execute if predicate matcha:not_in_village run advancement revoke @p only matcha:mechanics/enter_village
execute at @a run execute if score @p eerie >= 1 eerie run execute if predicate matcha:not_in_village run scoreboard players set @p eerie 0

#Check if the player is in a village strucutre, if so, let the entity face them
execute at @n[type=minecraft:mannequin,tag=haunted] run execute if score @p eerie >= 1 eerie run function matcha:environmental/village_entity_movement
#If the player is ina  village structure, and close to him, play Dry Hands
execute at @n[type=minecraft:mannequin,tag=haunted,tag=!music_played] run execute if score @p[distance=..30,gamemode=survival] eerie >= 1 eerie run function matcha:environmental/play_village_jukebox
#If they get too close, and don't come through the door, kill everything
execute at @n[type=minecraft:mannequin,tag=haunted] run execute at @n[type=marker,tag=jukebox] run execute if entity @p[distance=..2,gamemode=survival] run function matcha:environmental/kill_village_entity
execute at @n[type=minecraft:mannequin,tag=haunted] run execute if entity @p[distance=..5,gamemode=survival] run function matcha:environmental/kill_village_entity
