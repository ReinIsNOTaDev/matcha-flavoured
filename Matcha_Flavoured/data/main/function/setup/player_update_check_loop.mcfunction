# This was done to support multiplayer called by main:setup/load
...
execute as @a if score @s version_number < current_version version_number run function main:setup/update_this_player

# Run this checker again in one second's time
schedule function main:setup/player_update_check_loop 1s replace